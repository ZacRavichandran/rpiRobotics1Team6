...
class LineDetectorLoggingNode(object):
    def __init__(self):
        self.node_name = "LineDetectorLoggingNode"
...
#             self.detector_config = c
    ...
    def cbImage(self, image_msg):
        ...
        if not self.active:
            return 
        rospy.loginfo("line_detector_logging_node received callback")
...
class Stats():
    ...
    def received(self):
        if self.nreceived == 0 and self.nresets == 1:
            rospy.loginfo('line_detector_logging_node received first image.')
        self.nreceived += 1
    ...
    def processed(self):
        if self.nprocessed == 0 and self.nresets == 1:
            rospy.loginfo('line_detector_logging_node processing first image.')
...
if __name__ == '__main__': 
    rospy.init_node('line_detector_logging',anonymous=False)
    line_detector_node = LineDetectorLoggingNode()
